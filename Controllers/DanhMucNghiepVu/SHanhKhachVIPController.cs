...
{
    public class SHanhKhachVIPController : BaseController
    {
        // GET: SHanhKhachVIP
        ...
        public JsonResult GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = dbXNC.SHanhKhachVIPs.AsNoTracking().Where(x => true);
        ...
        public JsonResult GetItem(int id)
        {
            ...
            try
            {
                var item = dbXNC.SHanhKhachVIPs.Find(id);
                if (item == null)
                {
                    httpMessage.IsOk = false;
        ...
        public JsonResult Create(SHanhKhachVIP item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
        ...
        public JsonResult Update(SHanhKhachVIP item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                var exist = dbXNC.SHanhKhachVIPs.Find(item.Id);
                if (exist == null)
                {
                    httpMessage.IsOk = false;
                    httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Không tìm thấy thông tin");
        ...
        public JsonResult Delete(int id)
        {
            ...
            try
            {
                ...
                dbXNC.SHanhKhachVIPs.Remove(item);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public JsonResult ImportExcel()
        {
            ...
            try
            {
                ...
                var lstItem = new List<SHanhKhachVIP>();
                ...
                var lstItemNew = from a in lstItem
                                 join b in dbXNC.SHanhKhachVIPs.Select(x => x.SoGiayTo) on a.SoGiayTo equals b into c
              ...
              .ToList();
                ...
                dbXNC.SHanhKhachVIPs.AddRange(lstItem);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        private HttpMessage CheckValid(SHanhKhachVIP item)
        {
            ...
            try
            {
                ...
                var exist = dbXNC.SHanhKhachVIPs.AsNoTracking().FirstOrDefault(x => x.Id != item.Id && x.SoGiayTo == item.SoGiayTo);
                if (exist != null)
                {
