...
namespace WebNoiBai.Controllers.DanhMucNghiepVu
{
    public class SHanhKhachDiLaiNhieuController : BaseController
    {
        ...
        public JsonResult GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = dbXNC.SHanhKhachDiLaiNhieux.AsNoTracking().Where(x => true);
        ...
        public JsonResult GetItem(int id)
        {
            ...
            try
            {
                var item = dbXNC.SHanhKhachDiLaiNhieux.Find(id);
                if (item == null)
                {
                    httpMessage.IsOk = false;
        ...
        public JsonResult Create(SHanhKhachDiLaiNhieu item)
        {
            ...
            try
            {
                ...
                dbXNC.SHanhKhachDiLaiNhieux.Add(item);
                dbXNC.SaveChanges();
        ...
        public JsonResult Update(SHanhKhachDiLaiNhieu item)
        {
            ...
            try
            {
                var exist = dbXNC.SHanhKhachDiLaiNhieux.Find(item.Id);
                if (exist == null)
                {
                    httpMessage.IsOk = false;
                    httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Không tìm thấy thông tin");
        ...
        public JsonResult Delete(int id)
        {
            ...
            try
            {
                ...
                dbXNC.SHanhKhachDiLaiNhieux.Remove(item);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        private HttpMessage CheckValid(SHanhKhachDiLaiNhieu item)
        {
            ...
            try
            {
                ...
                var exist = dbXNC.SHanhKhachDiLaiNhieux.AsNoTracking().FirstOrDefault(x => x.Id != item.Id && x.SoGiayTo == item.SoGiayTo);
                if (exist != null)
                {
