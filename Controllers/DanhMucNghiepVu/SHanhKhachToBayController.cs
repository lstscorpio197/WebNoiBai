...
{
    public class SHanhKhachToBayController : BaseController
    {
        // GET: SHanhKhachToBay
        ...
        public JsonResult GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = dbXNC.SThanhVienToBays.AsNoTracking().Where(x => true);
        ...
        public JsonResult GetItem(int id)
        {
            ...
            try
            {
                var item = dbXNC.SThanhVienToBays.Find(id);
                if (item == null)
                {
                    httpMessage.IsOk = false;
        ...
        public JsonResult Create(SThanhVienToBay item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                ...
                dbXNC.SThanhVienToBays.Add(item);
                dbXNC.SaveChanges();
        ...
        public JsonResult Update(SThanhVienToBay item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                var exist = dbXNC.SThanhVienToBays.Find(item.Id);
                if (exist == null)
                {
                    httpMessage.IsOk = false;
                    httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Không tìm thấy thông tin");
        ...
        public JsonResult Delete(int id)
        {
            ...
            try
            {
                ...
                dbXNC.SThanhVienToBays.Remove(item);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public JsonResult ImportExcel()
        {
            ...
            try
            {
                ...
                var lstItem = new List<SThanhVienToBay>();
                for (int i = 2; i <= rows; i++)
                {
                    ...
                    value = value.Replace(";", "");
                    lstItem.Add(new SThanhVienToBay
                    {
                        SoGiayTo = value,
                        GhiChu = ghichu
                    });
                ...
                var lstItemNew = from a in lstItem
                                 join b in dbXNC.SThanhVienToBays.Select(x => x.SoGiayTo) on a.SoGiayTo equals b into c
              ...
              .ToList();
                ...
                dbXNC.SThanhVienToBays.AddRange(lstItem);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        private HttpMessage CheckValid(SThanhVienToBay item)
        {
            ...
            try
            {
                ...
                var exist = dbXNC.SThanhVienToBays.AsNoTracking().FirstOrDefault(x => x.Id != item.Id && x.SoGiayTo == item.SoGiayTo);
                if (exist != null)
                {
