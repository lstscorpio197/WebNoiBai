...
namespace WebNoiBai.Controllers.DanhMucNghiepVu
{
    public class SDoiTuongDaKTController : BaseController
    {
        ...
        public JsonResult GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = dbXNC.SDoiTuongDaKTs.AsNoTracking().Where(x => true);
        ...
        public JsonResult GetItem(int id)
        {
            ...
            try
            {
                var item = dbXNC.SDoiTuongDaKTs.Find(id);
                if (item == null)
                {
                    httpMessage.IsOk = false;
        ...
        public JsonResult Create(SDoiTuongDaKT item)
        {
            ...
            try
            {
                ...
                item.LichSuKiemTras = new List<LichSuKiemTra>
                { new LichSuKiemTra
                    {
                        KetQuaKT = item.KetQuaKT,
                        NgayKT = item.NgayKT,
                        NgayTao = DateTime.Now,
                        NguoiTao= us.Username
                    }
                };
                dbXNC.SDoiTuongDaKTs.Add(item);
                dbXNC.SaveChanges();
        ...
        public JsonResult Update(SDoiTuongDaKT item)
        {
            ...
            using (var trans = dbXNC.Database.BeginTransaction())
            {
                try
                {
                    var exist = dbXNC.SDoiTuongDaKTs.Find(item.Id);
                    if (exist == null)
                    {
                        httpMessage.IsOk = false;
                        httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Không tìm thấy thông tin");
                    ...
                    var existHis = dbXNC.LichSuKiemTras.Any(x => x.DoiTuongId == exist.Id);
                    bool updateKQ = exist.KetQuaKT != item.KetQuaKT || exist.NgayKT != item.NgayKT || (!existHis && !string.IsNullOrEmpty(item.KetQuaKT));
                    ...
                    if (updateKQ) {
                        ...
                        dbXNC.LichSuKiemTras.Add(newHis);
                        dbXNC.SaveChanges();
                    }
                    trans.Commit();
        ...
        public JsonResult Delete(int id)
        {
            ...
            try
            {
                ...
                dbXNC.SDoiTuongDaKTs.Remove(item);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public JsonResult ImportExcel()
        {
            ...
            try
            {
                ...
                var lstItem = new List<SDoiTuongDaKT>();
                ...
                var lstItemNew = from a in lstItem
                                 join b in dbXNC.SDoiTuongDaKTs.Select(x => x.SoGiayTo) on a.SoGiayTo equals b into c
              ...
              .ToList();
                ...
                dbXNC.SDoiTuongDaKTs.AddRange(lstItem);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public JsonResult GetListHistory(decimal id)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                var lstHis = dbXNC.LichSuKiemTras.Where(x=>x.DoiTuongId == id).ToList();
                httpMessage.Body.Data = lstHis;
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        private HttpMessage CheckValid(SDoiTuongDaKT item)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(item.KetQuaKT))
                {
                    httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Vui lòng nhập kết quả kiểm tra");
                    return httpMessage;
                }
                var exist = dbXNC.SDoiTuongDaKTs.AsNoTracking().FirstOrDefault(x => x.Id != item.Id && x.SoGiayTo == item.SoGiayTo);
                if (exist != null)
                {
