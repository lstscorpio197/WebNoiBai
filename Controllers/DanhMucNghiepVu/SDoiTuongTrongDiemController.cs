...
{
    public class SDoiTuongTrongDiemController : BaseController
    {
        // GET: SDoiTuongTrongDiem
        ...
        public JsonResult GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = dbXNC.SDoiTuongTrongDiems.AsNoTracking().Where(x => true);
        ...
        public JsonResult GetItem(int id)
        {
            ...
            try
            {
                var item = dbXNC.SDoiTuongTrongDiems.Find(id);
                if (item == null)
                {
                    httpMessage.IsOk = false;
        ...
        public JsonResult Create(SDoiTuongTrongDiem item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                ...
                dbXNC.SDoiTuongTrongDiems.Add(item);
                dbXNC.SaveChanges();
        ...
        public JsonResult Update(SDoiTuongTrongDiem item)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            try
            {
                var exist = dbXNC.SDoiTuongTrongDiems.Find(item.Id);
                if (exist == null)
                {
                    httpMessage.IsOk = false;
                    httpMessage.Body.MsgNoti = new HttpMessageNoti("400", null, "Không tìm thấy thông tin");
        ...
        public JsonResult Delete(int id)
        {
            ...
            try
            {
                ...
                dbXNC.SDoiTuongTrongDiems.Remove(item);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public JsonResult ImportExcel()
        {
            ...
            try
            {
                ...
                var lstItem = new List<SDoiTuongTrongDiem>();
                for (int i = 2; i <= rows; i++)
                {
                    ...
                    value = value.Replace(";", "");
                    lstItem.Add(new SDoiTuongTrongDiem
                    {
                        SoGiayTo = value,
                        GhiChu = ghichu
                    });
                ...
                var lstItemNew = from a in lstItem
                                 join b in dbXNC.SDoiTuongTrongDiems.Select(x => x.SoGiayTo) on a.SoGiayTo equals b into c
              ...
              .ToList();
                ...
                dbXNC.SDoiTuongTrongDiems.AddRange(lstItem);
                dbXNC.SaveChanges();
                return Json(httpMessage, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        private HttpMessage CheckValid(SDoiTuongTrongDiem item)
        {
            ...
            try
            {
                ...
                var exist = dbXNC.SDoiTuongTrongDiems.AsNoTracking().FirstOrDefault(x => x.Id != item.Id && x.SoGiayTo == item.SoGiayTo);
                if (exist != null)
                {
